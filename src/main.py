...
def compile_file(filepath):
    ...
    # -fsyntax-only: check syntax without producing output file
    # More reliable than '-o NUL' on Windows where stderr can get lost.
    result = subprocess.run(
        ['gcc', '-Wall', '-fsyntax-only', filepath],
...
def print_output(errors, dataset, filename):
    ...
    for i, err in enumerate(errors, 1):
        ...
        if entry:
            ...
            # Fallback if error not in dataset
            explanation = f"GCC reported: {err['msg']}"
            fix         = "Check the highlighted line carefully."
            secure_flag = "[Not in dataset]"
